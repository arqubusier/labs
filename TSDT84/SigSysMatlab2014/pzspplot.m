...
if type=='PZS' | type=='P&Z'	% Plot pz
   ...
   if todo=='addpz'		% Add extra pole or zero 
	...
	if length(presx)>0	                % => Present pole/zero exists
	   if userdata(19)==1, prtype='xb'; psz=2;
	   ...
	   userdata(20)=userdata(19); % Update previous pointer mode
	   userdata(21)=presx;        % Update the
	   userdata(22)=presy; 	      % previous pole/zero
	end
	userdata(19)=pmode;			% Update present type
   ...
   elseif todo=='Fulpl'	| todo=='PZmen'	% Default pz plot or plot after pzmenu
	 ...
	 if userdata(18)==1	% indicates z-transform
	    range=max(1,range);
	    t=0:.01:2*pi;
	    pp=plot(exp(j*t),'-');
	    set(pp,'color',[.7 .7 .7])
	 ...
	 if length(presx)>0	                % => Present pole/zero exists
	    if userdata(19)==1, prtype=REDX; psz=2;
   ...
   set(userdata(23),'string',K);  % Update K
...
if type=='PZS' | type=='Spe';	% Plot spectrum
   ...
   if todo=='Fulpl'  		% Use default (Full) frequency scaling
      fmin=0;     	
      if userdata(18)==0	% => Laplace transform
       	 fmax=bandwi(H);
   ...
   if userdata(17)==1,			% Linear magn. scaling
